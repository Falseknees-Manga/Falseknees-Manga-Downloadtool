    ...
    def save(self, use_indent=True):
        with open(self.path, 'w', encoding='utf-8') as f:
            if use_indent:
                ...
                json_dump(self.copy(), f, separators=(
                    ',', ':'), ensure_ascii=False)
...
if segmented_remainder != 0:
    ...
    config.save()
    logger.warning(
        '自动设置为 ' + str(config['number_of_segmented_download_thread']))
        ...
        try:
            ...
            if config['http_proxy']['enable']:
                response = requests_get(
                    link, headers=requests_header, verify=False, proxies=proxies)
            else:
                response = requests_get(
                    link, headers=requests_header, verify=False)
            response.raise_for_status()
        except Exception as e:
            try_times += 1
            logger.warning(
                f'{link} 加载失败[{try_times}/{times_limit}]，正在重新获取：{e}')
        else:
            if bin:
...
class download_thread(Thread):
    ...
    def exit(self):
        ...
        logger.warning(
            f'下载线程{self.name}关闭，完成任务：{self.completed}/{self.task_length}')
...
def segmented_downloader(target_thread):
    ...
    for i in task_list[threadID - 1]:
        ...
        image = requests(image_link, times_limit,
                         True, {"Range": header_range})
        if image == 'error':
            target_thread.has_error = True
            continue
        try:
...
def get_content_length(target_thread):
    ...
    for i in task_list[threadID - 1]:
            ...
            try:
                if config['http_proxy']['enable']:
                    response = requests_head(
                        'https://falseknees.com/' + i[1], verify=False, proxies=proxies).headers['Content-Length']
                else:
                    response = requests_head(
                        'https://falseknees.com/' + i[1], verify=False).headers['Content-Length']
            except Exception as e:
                try_times += 1
                logger.warning(
                    f'{i[1]} 加载失败[{try_times}/{times_limit}]，正在重新获取：{e}')
            else:
        ...
        if not os_path_exists(i[0] + '.' + suffix):
            open(os_path_join(config['folder_path'],
                              i[0] + '.' + suffix), 'w').close
...
def task_assignment(task, log: bool = False):
    ...
    if log:
        for i in range(1, config['number_of_thread'] + 1):
            ...
            infomation.append(
                f' - download-thread-{i} 拥有任务数：{len(new_task_list[i - 1])}')
...
def start_download_thread(function_used, inherit=True):
    ...
    for i in range(1, config['number_of_thread'] + 1):
        download_thread_list.append(download_thread(
            function_used, i, config['retry_times']))
...
def normal_start():
    ...
    archive = requests('https://falseknees.com/archive.html',
                       config['retry_times'])
    if archive == 'error':
...
class Console(Thread):
    ...
    def cmd_thread_parser(self):
        if self.cmd[1] == 'list':
            ...
            global download_thread_list
            logger.critical(
                '您输入了一条隐藏的Debug指令，这会导致所有下载线程强制退出，如果线程中涉及获取释放锁，这可能会导致死锁')
